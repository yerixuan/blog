...
function _mediaEmbed(attrs) {
	...
	//这里修改 视频 播放方式 用于兼容 不同浏览器
	
	console.log("5566 "+attrs.src);
	console.log(attrs);
	
	var media_src=attrs.src;
...
KEditor.prototype = {
	...
	text : function(val) {
		...
		if (val === undefined) {
			...
			console.log("77="+self.html());
			return _trim(self.html().replace(/<(?!img|embed).*?>/ig, '').replace(/&nbsp;/ig, ' '));
		} else {
			console.log("75="+self.html());
...
_plugin('core', function(K) {
	...
	self.beforeSetHtml(function(html) {
		...
		return html.replace(/<embed[^>]*type="([^"]+)"[^>]*>(?:<\/embed>)?/ig, function(full) {
			...
			console.log("草泥马");
			console.log(_mediaImg(self.themesPath + 'common/blank.gif', attrs))
